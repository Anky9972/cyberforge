...
export class CoverageGuidedFuzzingEngine {
     ...
     */
    async fuzzFunctionWithCoverage(code, functionName, paramTypes) {
     ...
     */
    generateInitialInput(paramTypes) {
     ...
     */
    mutateInterestingInput(input, paramTypes, count = 10) {
        const mutations = [];
        for (let i = 0; i < count; i++) {
            const mutated = [...input];
            const indexToMutate = Math.floor(Math.random() * input.length);
            mutated[indexToMutate] = this.mutateValue(mutated[indexToMutate], paramTypes[indexToMutate]);
...
//# sourceMappingURL=coverageGuidedFuzzing.js.map