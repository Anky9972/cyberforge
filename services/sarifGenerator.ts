...
class SARIFGeneratorService {
  private readonly SARIF_VERSION = '2.1.0';
   ...
   */
  private createRule(vuln: any): SARIFRule {
   ...
   */
  private createResult(vuln: any): SARIFResult {
   ...
   */
  private createLocations(vuln: any): SARIFLocation[] {
    if (!vuln.location) {
      return [];
    }
   ...
   */
  private calculateSecuritySeverity(severity: string): number {
    const sev = severity?.toLowerCase() || '';
    
    if (sev.includes('critical')) return 10.0;
    if (sev.includes('high')) return 8.0;
    if (sev.includes('medium')) return 5.0;
    if (sev.includes('low')) return 2.0;
    return 0.0;
  }
   ...
   */
  private calculateCVSS(vuln: any): number {
    let score = 5.0; // Base score

    // Increase for high severity
    if (vuln.severity?.toLowerCase().includes('critical')) score += 4.0;
    else if (vuln.severity?.toLowerCase().includes('high')) score += 2.0;
   ...
   */
  private calculateExploitability(vuln: any): number {
    let score = 50; // Base

    // Has PoC?
    if (vuln.poc || vuln.input) score += 30;

    // Easy to trigger?
    if (vuln.complexity === 'low') score += 10;

    // Remote?
    if (vuln.vector === 'network') score += 10;
   ...
   */
  private extractTags(vuln: any): string[] {
   ...
   */
  private inferAttackTags(vuln: any): string[] {
    const tags: string[] = [];
   ...
   */
  private generateFingerprint(vuln: any): string {
