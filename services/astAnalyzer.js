...
export class ASTAnalyzer {
     ...
     */
    fallbackAnalysis(code, filename, language) {
        const nodes = [];
        const edges = [];
    ...
    // Helper methods
    getClassName(path) {
        let parentClass = path.findParent((p) => t.isClassDeclaration(p.node));
     ...
     */
    analyzeJavaScriptCode(code, filename) {
        try {
            ...
            const uniqueEdges = Array.from(new Set(edges.map(e => JSON.stringify(e)))).map(s => JSON.parse(s));
            return {
     ...
     */
    findSecurityPatterns(code, language) {
    ...
    findJavaScriptSecurityPatterns(code) {
        const patterns = [];
        try {
            const ast = parser.parse(code, {
                sourceType: 'module',
    ...
    findPythonSecurityPatterns(code) {
        const patterns = [];
