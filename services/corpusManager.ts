...
export interface Seed {
  ...
  coverageScore: number; // Unique edges/blocks covered
  crashCount: number; // Number of crashes found using this seed
  energy: number; // Fuzzing energy (priority)
  isGolden: boolean; // Manually promoted high-value seed
  metadata: {
...
export interface Corpus {
  ...
  totalCoverage: Set<string>; // All unique edges covered
  createdAt: Date;
  updatedAt: Date;
  stats: {
    totalSeeds: number;
...
class CorpusManagerService {
  ...
  private corpusDir: string = './corpus';

  constructor(corpusDir?: string) {
    if (corpusDir) this.corpusDir = corpusDir;
  }

  /**
   * Initialize corpus for a target
   */
  async initializeCorpus(targetId: string): Promise<Corpus> {
  ...
  async addSeed(
    targetId: string,
    content: string | Buffer,
    metadata: Partial<Seed['metadata']> = {}
  ): Promise<Seed> {
   ...
   */
  async promoteToGolden(targetId: string, seedId: string): Promise<boolean> {
    const corpus = this.corpora.get(targetId);
   ...
   */
  async minimizeCorpus(targetId: string): Promise<SeedEvolutionMetrics> {
    const corpus = this.corpora.get(targetId);
    if (!corpus) throw new Error(`Corpus not found: ${targetId}`);
    ...
    const seedCoverageMap = new Map<string, Set<string>>();
    ...
    const keptSeeds = new Set<string>();
   ...
   */
  private simulateSeedCoverage(seed: Seed): Set<string> {
   ...
   */
  private calculateAverageCoverageScore(corpus: Corpus): number {
    if (corpus.seeds.size === 0) return 0;
   ...
   */
  private calculateAverageEnergy(corpus: Corpus): number {
    if (corpus.seeds.size === 0) return 0;
   ...
   */
  getSeedsByEnergy(targetId: string, limit: number = 100): Seed[] {
    const corpus = this.corpora.get(targetId);
    if (!corpus) return [];
   ...
   * Save corpus to disk
   */
  private async saveCorpusToDisk(targetId: string): Promise<void> {
    const corpus = this.corpora.get(targetId);
    if (!corpus) return;
   ...
   */
  private async loadCorpusFromDisk(targetId: string): Promise<void> {
    const targetDir = path.join(this.corpusDir, targetId);
    const metadataPath = path.join(targetDir, 'corpus.json');
   ...
   */
  async exportCorpus(targetId: string): Promise<object> {
    const corpus = this.corpora.get(targetId);
    if (!corpus) throw new Error(`Corpus not found: ${targetId}`);
   ...
   * Import corpus
   */
  async importCorpus(data: any): Promise<void> {
