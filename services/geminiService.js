import { CKGDataSchema, ReconResponseSchema, APIResponseSchema, FuzzTargetResponseSchema, VulnerabilityReportSchema, detectPII } from './schemas';
import { ASTAnalyzer } from './astAnalyzer';
import { JavaScriptFuzzingEngine, generateVulnerabilityFromFuzz } from './fuzzingEngine';
...
function extractJSON(text) {
    ...
    if (jsonStart !== -1 && jsonEnd !== -1 && jsonEnd > jsonStart) {
        ...
        console.log("✅ Extracted JSON boundaries successfully");
    }
...
function cleanAndFixJSON(jsonStr) {
    console.log("🧹 Applying comprehensive JSON cleaning...");
    let cleaned = jsonStr;
    ...
    try {
        ...
        console.warn("⚠️ String cleaning failed, using basic cleaning");
    }
    ...
    try {
        ...
        console.warn("⚠️ JSON still invalid after cleaning, applying fallback cleanup");
...
async function callMistralAPI(systemPrompt, userPrompt, responseFormat, retryCount = 0) {
...
export async function generateCKG(codebaseSummary) {
 ...
 */
export async function generateCKGWithAST(codebaseSummary, codeFiles) {
    ...
    const uniqueEdges = Array.from(new Set(allEdges.map(e => JSON.stringify(e)))).map(s => JSON.parse(s));
...
export async function performReconnaissanceAnalysis(codebaseSummary) {
    ...
    try {
        ...
        while (parseAttempts < maxAttempts) {
            try {
                ...
                console.warn(`⚠️ Parse attempt ${parseAttempts} failed:`, parseError instanceof Error ? parseError.message : parseError);
                if (parseAttempts < maxAttempts) {
                    // Try more aggressive cleaning for next attempt
                    console.log("🔧 Applying more aggressive JSON cleaning...");
                    ...
                    if (parseAttempts === 1) {
                        ...
                        try {
                            ...
                            console.warn("⚠️ JSON rebuild failed:", rebuildError);
                        }
                    }
                }
                else {
                    ...
                    console.log("🔄 Returning empty findings due to parsing failure");
                    return [];
                }
            }
        }
        if (!validated) {
            console.log("🔄 JSON parsing completely failed, returning empty findings");
            return [];
        }
        console.log("✅ Reconnaissance analysis completed successfully");
        ...
        if (error instanceof Error) {
            if (error.message.includes('timed out')) {
                ...
                console.log("🔄 Schema validation failed, returning empty findings");
                return [];
            }
        ...
        console.log("🔄 Unexpected error occurred, returning empty findings");
        return [];
    }
}
...
export async function performAPISecurityAnalysis(codebaseSummary) {
    ...
    try {
        ...
        while (parseAttempts < maxAttempts) {
            try {
                parseAttempts++;
                console.log(`🔄 API Security JSON Parse attempt ${parseAttempts}/${maxAttempts}`);
                const parsedData = JSON.parse(cleanedJSON);
                ...
                console.log("✅ API Security JSON parsed and validated successfully");
                break;
            }
            catch (parseError) {
                console.warn(`⚠️ API Security parse attempt ${parseAttempts} failed:`, parseError instanceof Error ? parseError.message : parseError);
                if (parseAttempts < maxAttempts) {
                    // Apply the same progressive cleaning strategies
                    console.log("🔧 Applying more aggressive JSON cleaning for API Security...");
                    if (parseAttempts === 1) {
                        ...
                        try {
                            ...
                            console.warn("⚠️ API Security JSON rebuild failed:", rebuildError);
                        }
                    }
                }
                else {
                    // All attempts failed
                    console.error("❌ All API Security JSON parsing attempts failed");
                    console.error("Final cleaned JSON:", cleanedJSON.substring(0, 1000));
                    // Return empty findings instead of throwing
                    console.log("🔄 Returning empty API Security findings due to parsing failure");
                    return [];
                }
            }
        }
        if (!validated) {
            console.log("🔄 API Security JSON parsing completely failed, returning empty findings");
            return [];
        }
        console.log("✅ API Security analysis completed successfully");
        return (validated.findings || []);
    }
    catch (error) {
        ...
        if (error instanceof Error) {
            if (error.message.includes('timed out')) {
                ...
                console.log("🔄 API Security schema validation failed, returning empty findings");
                return [];
            }
            else if (error.message.includes('AbortError')) {
        ...
        // For any other error, return empty findings instead of failing completely
        console.log("🔄 Unexpected API Security error occurred, returning empty findings");
        return [];
    }
}
...
export async function identifyFuzzTargets(ckgSummary) {
 ...
 */
function generateDefaultFuzzTargets(ckgSummary) {
    ...
    const explicitFunctions = new Set();
    let match;
 ...
 */
function detectLanguageFromCKG(ckgSummary) {
    const summary = ckgSummary.toLowerCase();
    if (summary.includes('python') || summary.includes('.py') || summary.includes('def '))
        return 'Python';
    if (summary.includes('javascript') || summary.includes('.js') || summary.includes('function'))
        return 'JavaScript';
    if (summary.includes('typescript') || summary.includes('.ts'))
        return 'TypeScript';
    if (summary.includes('java') || summary.includes('.java'))
        return 'Java';
    if (summary.includes('c++') || summary.includes('.cpp') || summary.includes('.hpp'))
        return 'C++';
    if (summary.includes('golang') || summary.includes('.go'))
        return 'Go';
    if (summary.includes('rust') || summary.includes('.rs'))
        return 'Rust';
    if (summary.includes('.c') || summary.includes('.h'))
        return 'C';
...
`;
export async function generatePromptFuzzInputs(fuzzTargets) {
 ...
 */
function generateDefaultFuzzInputs(fuzzTargets) {
...
`;
export async function simulateFuzzingAndGenerateReport(ckgSummary, reconFindings, apiFindings, fuzzTargets, fuzzInputs) {
 ...
 */
export async function executeRealFuzzingAndGenerateReport(codeFiles, ckgSummary, reconFindings, apiFindings, fuzzTargets) {
    ...
    const hasJavaScriptCode = Array.from(codeFiles.values()).some(file => file.language === 'JavaScript' || file.language === 'TypeScript');
    ...
    if (!hasJavaScriptCode || (primaryLanguage !== 'JavaScript' && primaryLanguage !== 'TypeScript')) {
        ...
        return await simulateFuzzingAndGenerateReport(ckgSummary, reconFindings, apiFindings, fuzzTargets, `Real fuzzing engine unavailable (language incompatibility: ${primaryLanguage} codebase requires ${primaryLanguage} fuzzing engine)`);
    }
    ...
    return await simulateFuzzingAndGenerateReport(ckgSummary, reconFindings, apiFindings, fuzzTargets, fallbackReason);
}
