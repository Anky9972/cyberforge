 ...
 */
async function executeFuzzing(target) {
 ...
 */
function mutateSeed(seed, iteration) {
    const mutations = [
        // Bit flip
        () => {
...
async function executeCode(code, input, language) {
    try {
        ...
        // Simulate coverage
        const coverage = Array.from({ length: Math.floor(Math.random() * 10) }, (_, i) => i);
        return {
            crashed: false,
            coverage
        };
 ...
 */
function detectSimpleVulnerabilities(code, input) {
...
//# sourceMappingURL=fuzzWorker.js.map