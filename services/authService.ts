...
export class AuthService {
   ...
   * Login user
   */
  async login(emailOrUsername: string, password: string, ipAddress?: string, userAgent?: string): Promise<AuthTokens & { user: any }> {
    // Find user
  ...
  async refreshToken(refreshToken: string): Promise<AuthTokens> {
    ...
    let decoded: any;
    try {
      decoded = jwt.verify(refreshToken, this.jwtRefreshSecret) as TokenPayload;
    } catch (error) {
   ...
   * Create API key
   */
  async createApiKey(userId: string, name: string, permissions: string[], expiresAt?: Date): Promise<{ id: string; key: string }> {
    // Generate API key
