...
class AutoFixGeneratorService {
   ...
   */
  async generateFix(vulnerability: Vulnerability, fullCode?: string): Promise<AutoFixResult> {
    if (!this.model) {
   ...
   */
  private buildFixPrompt(vulnerability: Vulnerability, fullCode?: string): string {
...
Generate the fix now:`;
   ...
   */
  private parseFixResponse(response: string, originalCode: string): AutoFixResult {
    try {
   ...
   */
  private generateFallbackFix(vulnerability: Vulnerability): AutoFixResult {
  ...
  private fixXSS(code: string): string {
    return code.replace(
      /innerHTML\s*=\s*(\w+)/g,
      'textContent = $1 // or use DOMPurify.sanitize($1)'
    );
  }
  ...
  private fixCommandInjection(code: string): string {
    return code.replace(
      /exec\s*\(\s*['"`].*?\$\{(\w+)\}.*?['"`]\s*\)/g,
      'execFile("command", [$1], { shell: false })'
    );
  }
...
${code}`;
   ...
   */
  private addGenericSecurity(code: string): string {
...
${code.replace(/input/g, 'sanitized')}`;
   ...
   */
  private generateSQLInjectionTests(): string {
    return `
describe('SQL Injection Fix', () => {
   ...
   * Generate XSS tests
   */
  private generateXSSTests(): string {
    return `
describe('XSS Fix', () => {
   ...
   */
  private generateCommandInjectionTests(): string {
    return `
describe('Command Injection Fix', () => {
   ...
   */
  private generatePathTraversalTests(): string {
    return `
describe('Path Traversal Fix', () => {
   ...
   */
  async generateBatchFixes(vulnerabilities: Vulnerability[]): Promise<AutoFixResult[]> {
