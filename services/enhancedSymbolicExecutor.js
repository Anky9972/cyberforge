...
class EnhancedSymbolicExecutor {
     ...
     */
    extractConstraints(code, language = 'javascript') {
        const constraints = [];
        try {
     ...
     */
    async analyzePathConstraints(constraints) {
        if (!this.initialized) {
        ...
        try {
            ...
            for (const constraint of constraints) {
                ...
                try {
                    ...
                    const z3Constraint = this.parseConstraintToZ3(constraint.condition, variables, this.z3Context);
                    if (z3Constraint) {
     ...
     */
    parseConstraintToZ3(condition, variables, ctx) {
        try {
     ...
     */
    async findInputsForLocation(code, targetLine) {
...
function extractVariables(node, vars = new Set()) {
    ...
    if (node.argument)
        extractVariables(node.argument, vars);
    if (node.test)
        extractVariables(node.test, vars);
    return vars;
}
export const enhancedSymbolicExecutor = new EnhancedSymbolicExecutor();
export default EnhancedSymbolicExecutor;
//# sourceMappingURL=enhancedSymbolicExecutor.js.map