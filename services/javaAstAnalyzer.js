...
export class JavaASTAnalyzer {
     ...
     */
    extractClassInfo(classDecl, nodes, edges) {
            ...
            if (classBody?.children?.classBodyDeclaration) {
                ...
                for (const decl of declarations) {
                    if (decl.children?.classMemberDeclaration?.children?.methodDeclaration) {
                        this.extractMethodInfo(decl.children.classMemberDeclaration.children.methodDeclaration, className, nodes, edges);
                    }
                }
            }
        }
     ...
     */
    extractMethodInfo(methodDecl, className, nodes, edges) {
     ...
     */
    extractMethodCalls(methodBody, currentClass) {
        const calls = [];
     ...
     */
    fallbackJavaAnalysis(code, filename) {
        const nodes = [];
        const edges = [];
        // Extract class names
...
//# sourceMappingURL=javaAstAnalyzer.js.map